...
async def tt_download2(message: types.Message):
    ...
    try:
        ...
        if response.endswith(".mp3"):
            await message.reply_audio(open(response, 'rb'), caption='@moviesandtvseriesssss', title=link)
        # video
        else:
            logging.info(f"VIDEO: {response}")
            await message.reply_video(open(response, 'rb'), caption='@moviesandtvseriesssss',)
        os.remove(response)
